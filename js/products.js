      ...
      <div class="col-xl-3 col-lg-3 col-md-6">
         <div class="product-wrapper text-center mb-30">
            ...
            <div class="product-text product2-text">
               <h4>
                  <a target="_blank" rel="noopener noreferrer" href="https://riofitmeals.dailykit.org/store/ProductPage?id=${id}&type=${type}">${name}</a>
               </h4>
               ${
                 Boolean(price)
                   ? `<div class="pro-price">
    ...
    if (success) {
      ...
      if (menu.length > 0) {
        ...
        categories.forEach(async (category) => {
          ...
          let products_listing_button = $(`[data-category-button='${name}']`);

          const types = {
            inventoryProducts: "INVENTORY_PRODUCT",
            simpleRecipeProducts: "SIMPLE_RECIPE_PRODUCT",
            comboProducts: "COMBO_PRODUCT",
            customizableProducts: "CUSTOMIZABLE_PRODUCT",
          };

          if (
            rest.simpleRecipeProducts.length === 0 &&
            rest.inventoryProducts.length === 0 &&
            rest.comboProducts.length === 0
          ) {
            ...
            products.forEach(async (id) => {
              ...
              if (success) {
                if (type === "simpleRecipeProducts") {
                  const { data: { simpleRecipeProduct = {} } = {} } = response;
                  products_listing_container.append(
                    product({
                      ...
                      image:
                        Array.isArray(simpleRecipeProduct.images) &&
                        simpleRecipeProduct.images.length > 0
                          ? simpleRecipeProduct.images[0]
                          : "",
                      ...(simpleRecipeProduct.defaultOption && {
                        price:
                          simpleRecipeProduct.defaultOption.price.length > 0
                            ? simpleRecipeProduct.defaultOption.price[0].value
                            : "",
                      }),
                    })
                  );
                } else if (type === "inventoryProducts") {
                  const { data: { inventoryProduct = {} } = {} } = response;
                  products_listing_container.append(
                    product({
                      ...
                      image:
                        Array.isArray(inventoryProduct.images) &&
                        inventoryProduct.images.length > 0
                          ? inventoryProduct.images[0]
                          : "",
                      ...(inventoryProduct.defaultOption && {
                        price:
                          inventoryProduct.defaultOption.price.length > 0
                            ? inventoryProduct.defaultOption.price[0].value
                            : "",
                      }),
                    })
                  );
                } else if (type === "comboProducts") {
                  const { data: { comboProduct = {} } = {} } = response;
                  products_listing_container.append(
                    product({
                      id: comboProduct.id,
                      ...
                      type: "comboProduct",
                      image:
                        Array.isArray(comboProduct.images) &&
                        comboProduct.images.length > 0
                          ? comboProduct.images[0]
                          : "",
                    })
                  );
                } else if (type === "customizableProducts") {
                  const { data: { customizableProduct = {} } = {} } = response;
                  products_listing_container.append(
                    product({
                      id: customizableProduct.id,
                      name: customizableProduct.name,
                      type: "customizableProduct",
                      image:
                        Array.isArray(customizableProduct.images) &&
                        customizableProduct.images.length > 0
                          ? customizableProduct.images[0]
                          : "",
                    })
                  );
                }
